...
export default async function LoginPage({ searchParams }: LoginPageProps) {
  ...
  return (
    <div className="flex min-h-dvh items-center justify-center px-4 py-6 sm:p-6">
      <div className="flex flex-col items-center gap-6 sm:gap-8 w-full max-w-[26rem] sm:max-w-sm">

        {/* Card */}
        <div className="w-full rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 p-6 sm:p-8 shadow-2xl flex flex-col items-center gap-5 sm:gap-6">

          {/* Logo + name */}
          <div className="flex flex-col items-center gap-4">
            <svg viewBox="0 0 1024 1024" className="h-14 w-14 sm:h-16 sm:w-16" fill="none">
            ...
            <div className="text-center">
              <h1 className="text-[1.7rem] sm:text-3xl font-bold text-white tracking-tight">Betinapp</h1>
              <p className="text-white/70 text-sm sm:text-sm mt-1">Making life at the coast easier, since 2026</p>
            </div>
          </div>
          ...
          <form action={async () => {
            ...
            <button
              type="submit"
              className="flex w-full items-center justify-center gap-3 rounded-xl bg-white px-4 py-3.5 text-sm font-semibold text-gray-800 shadow-sm hover:bg-white/90 active:scale-[0.98] transition-all cursor-pointer"
            >
